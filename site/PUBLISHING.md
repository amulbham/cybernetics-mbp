...
1. Frontmatter in `src/content/research/<slug>/index.md` (`format`, `title`, `pubDate`, `description`, `excerpt`, paper-only `pillar`, optional `subtitle` / `doi` / `tags` / hero pair / `readerNote`). Authors never specify a PDF field — a paper's PDF is always `{canonicalURL}{entry.id}.pdf`, derived (Sprint 8.4, filename T18.1). `/paper.pdf` in that directory is a 301 to that file, not a second PDF.
2. Body Markdown.
...
RESEARCH OBJECT
  ...
  └── PDF projection    {canonical}{entry.id}.pdf
                        citation_pdf_url, MediaObject.contentUrl, Download href
                        legacy {canonical}paper.pdf → 301, no second file
```

One Markdown source. One canonical HTML identity. One *derived* PDF URL — never a field to author, never a second identity that can drift from the first. `ResearchLayout.astro` computes `paperPdfUrl` once (`new URL(paperPdfFilename(id), canonicalURL)`, T18.1) and the three PDF consumers — `citation_pdf_url`, the JSON-LD `MediaObject.contentUrl`, and the reader-facing Download PDF link — read that same value. The article `url` / `mainEntityOfPage.@id` stay the HTML canonical URL. There is no `pdfUrl` frontmatter field (removed outright, `content.config.ts`) and no authored override. Authors never set a PDF field, full stop.
