...
- **`download="{title}.pdf"` on the Download PDF link** — today it just links to `paper.pdf`, so a saved file's name is whatever the URL's last segment is rather than the paper's actual title. Explicitly out of scope for Sprint 10 (flagged in its own kickoff). A real but small addition whenever it's worth doing.
