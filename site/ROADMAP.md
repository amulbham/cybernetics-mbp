...
- `npm audit` currently reports 6 vulnerabilities (1 moderate, 5 high) in transitive build-tool dependencies (`postcss` — sourceMappingURL path traversal; `svgo` — incomplete `removeScripts` fix). Both are build-time tooling, not runtime code shipped to site visitors, but never actually investigated or triaged — worth a look, `npm audit fix` may resolve them or may need a closer look if it doesn't.
