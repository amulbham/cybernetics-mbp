...
A typed, directional registry of how published pieces actually relate — not an autolinker, not a graph UI. Sprint 7 (2026-08-31, ongoing): 7.1 shipped the contract and an empty live file; 7.4 accepted the first placement; 7.5 shipped the transform that renders it.

- **Trigger**: none in Markdown. Source of truth is `src/data/research-relations.json`, not a content shape — this module doesn't pattern-match the body of any entry the way the other modules on this page do.
- **Produces**: for each `inline.status === 'accepted'` row whose `source` matches the current page, one ordinary `<a href={canonicalPath(target)}>` wrapped around the exact `anchor` string inside the section whose heading id is `inline.near` — no new class, an optional `data-relation="{id}"` for QA. A page with zero accepted rows for it renders byte-identical to before this module existed. The plugin renders the one decision already recorded in the registry; it never discovers, scores, or chooses a placement itself.
- **Exclusions** — a text node is never wrapped if it sits inside `a`, `h1`–`h6`, `table`, `pre`/`code`, `aside`, a `markdown-alert`, `section.references`, a `ref-N` element, or `figcaption`. Already wrapped in an `<a>` to the same href → no-op (an authored link always wins). Already wrapped in an `<a>` to a different href → build fails. The anchor must occur wholly inside exactly one eligible text node — zero occurrences, 2+ occurrences, or a phrase split across sibling markup (never reconstructed across nodes) all fail the build.
- **Fails closed, hard.** No skip-and-warn: an accepted row that can't be applied safely fails the build. This needed more than the plugin throwing — Astro's own content-layer glob loader catches a per-entry render error, logs it, and *continues*, leaving `astro build` at exit 0 with the affected page silently missing content (confirmed directly: a deliberately broken `near` produced a "successful" build with Three SOS's Introduction section entirely absent). `scripts/validate-research-links.mjs` runs first in `npm run build` specifically to close this gap — the exact same markdown pipeline, run standalone outside the glob loader's try/catch, so the same failure is a real uncaught exception with a non-zero exit code.
- **Citations are a separate system.** `#ref-*` links (`rehype-citation-links.mjs`) and this module's `<a>`s never compete for the same text in practice — this module only ever wraps the one recorded `anchor` string, and citation shapes (`(Author, Year)` / `Name (Year)`) don't collide with an arbitrary prose sentence by construction. Registered after `rehypeCitationLinks` regardless, so if they ever did overlap, the citation linker's claim on that text would already be settled first.
- **Files**: `src/lib/rehype-research-links.mjs` (the transform), `scripts/validate-research-links.mjs` (the build-fail guarantee), registered in `astro.config.mjs` after `rehypeCitationLinks` and an explicit `rehypeHeadingIds` (imported from `@astrojs/markdown-remark` — Astro's own default heading-id pass otherwise runs *after* every plugin in this array, too late for `near` to resolve; see `AGENTS.md`).
- **Row shape** (`src/lib/research-relations.ts`, a Zod schema — imported by both `rehype-research-links.mjs` and `scripts/validate-research-links.mjs` as of 7.5):
  ```
  {
    id: string,
...
- **Layered, not merged** — three separate systems that must never collapse into one:
  ...
  - `src/data/research-relation-projections.json` (Sprint 11.3/11.4) is a fifth, deliberately separate system: an explicit, human-authored policy for projecting one accepted row of *this* registry into external Schema.org JSON-LD (`isBasedOn`, v1). It never redefines `source`/`target`/`type`/`reason` — those stay authoritative here — and it is never derived from `type` or `reason`, nor gated by `inline.status`. Full contract: `PUBLISHING.md`'s "research object's semantic graph" section; standing invariants: `AGENTS.md`; history: `CHANGELOG.md`'s Sprint 11 entries.
- **Silence is a valid, permanent-until-earned state — not just historically, but as an ongoing live fact.** The frontier-training essay has zero rows naming it, in either direction: it was audited in Sprint 7.0 and found to have no honest relation to anything else in the corpus, and that isolation is the correct, standing result, not a page waiting its turn. Sprint 7 paused after 7.1 with the file at `[]`; Sprint 7.2 (2026-08-31) added the first live row, `three-sos--invariants`, once Three SOS actually landed in the collection. Pangram→FAFSA (`converges_with`) stays unadded until its own source entry exists — an empty *or* partially-populated file is correct at any point, never a placeholder waiting to be filled out to some target size.
- The live file may contain relation-only rows (no `inline` block) — that's the expected common case, not an incomplete one. First (and currently only) row: `three-sos--invariants`, `inline.status: "accepted"`, `near: "1-introduction"`, `anchor: "Paper A in the Cognitive Physics Series"` (the verbatim, globally-unique opening sentence of Three SOS's own §1 — audited in 7.3, verified against the built page before accepting). As of 7.5, this row is rendered: Three SOS's §1 carries exactly one `<a>` to Invariants; see "Produces" above.
- **Files**: `src/data/research-relations.json` (the live data — one row), `src/lib/research-relations.ts` (the Zod schema). The transform's own files are listed above.
