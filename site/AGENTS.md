...
- `rehype-references.mjs` — operates on the **hast** (HTML AST, after conversion), finds a heading whose text is exactly "References" followed by an `<ol>`, and wraps both in `<section class="references">` with `references-heading`/`references-list` classes for `global.css` to style. Restructuring/wrapping actual elements is a rehype job, not remark — mdast has no clean way to express an arbitrary wrapper element. **Gotcha already hit once**: the heading and list are *not* strictly adjacent siblings in the tree — there's a whitespace-only text node (a literal `"\n"`) between them from the blank line in the source Markdown, so any similar transform needs to skip whitespace text nodes when looking for "the next real sibling," not assume `children[i + 1]` is it.
...
Section-transform CSS (`.references*`, `.table-container`, `.markdown-alert*`) goes in `global.css`, unscoped — same reason `.prose p`/`code`/`pre`/`blockquote` are unscoped there: Astro's component-scoped `<style>` can't reach markup injected via `<slot />` from rendered Markdown, only markup written directly in that `.astro` file's own template.
