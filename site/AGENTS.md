...
Two examples, both in `src/lib/`, both plain functions with no new dependency:
...
Section-transform CSS (like `.references`/`.references-heading`/`.references-list`) goes in `global.css`, unscoped — same reason `.prose p`/`code`/`pre`/`blockquote` are unscoped there: Astro's component-scoped `<style>` can't reach markup injected via `<slot />` from rendered Markdown, only markup written directly in that `.astro` file's own template.
